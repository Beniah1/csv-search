...
async function searchRecords(searchTerm) {
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .or(
      `full_name.ilike.%${searchTerm}%,gender.ilike.%${searchTerm}%,phone_number.ilike.%${searchTerm}%`
    )
    .order("full_name");
...
function openEditModal(record) {
  ...
  document.getElementById("editCurrentLevel").value =
  ...
  results.forEach((record) => {
    ...
    const content = `
            ...
            <p><strong>Current Level:</strong> ${
              record.current_level || "Not Set"
            }</p>
            <p><strong>Attendance:</strong></p>
            <ul>
                <li>5th: <span class="attendance-status ${
                  record.attendance_5th || ""
                }">${record.attendance_5th || "Not Set"}</span></li>
                <li>12th: <span class="attendance-status ${
                  record.attendance_12th || ""
                }">${record.attendance_12th || "Not Set"}</span></li>
                <li>19th: <span class="attendance-status ${
                  record.attendance_19th || ""
                }">${record.attendance_19th || "Not Set"}</span></li>
                <li>26th: <span class="attendance-status ${
                  record.attendance_26th || ""
                }">${record.attendance_26th || "Not Set"}</span></li>
            </ul>
            <div class="button-group">
                <button onclick="openEditModal(${JSON.stringify(record).replace(
                  /"/g,
                  "&quot;"
...
(async () => {
  try {
    ...
    document.getElementById(
      "searchResults"
    ).innerHTML = `<p>Database Error: ${error.message}</p>`;
  }
})();
